        ...
        <div class="row en-container">
          <div class="col s12">
              <div class="card-panel white">
                <a href="{{URL::to('/dashboard/exportPlane')}}" class="btn green waves-effect btn-small"><i class="material-icons left">save</i>Export Excel</a>
                <a href="{{URL::to('/dashboard/addPlane')}}" class="btn waves-effect btn-small"><i class="material-icons left">add</i>Add New Plane</a>
                <a href="{{URL::to('/dashboard/addPlaneRoute')}}" class="btn amber waves-effect btn-small"><i class="material-icons left">add</i>Add New Route</a>
              </div>
          </div>
        </div>
                      ...
                      <tbody>
                       ...
                       @foreach ($dataPlane as $row)
                          <tr>
                            ...
                            <td>{{$row->seat_qty}}</td>
                            <td style="width: 90px">
                                <button class="col s4  btn btn-small yellow darken-2 waves-effect waves-light" data-toggle="modal" data-target="#editPlane" data-content="{{Crypt::encryptString($row->id)}}" ><i class="material-icons">edit</i></button>
                                <button class="col s4  btn btn-small red darken-2 waves-effect waves-light" data-toggle="modal" data-target="#delPlane" data-content="{{Crypt::encryptString($row->id)}}"><i class="material-icons">delete</i></button>
                            </td>
                          </tr>
                        @endforeach
                      </tbody>
...
@section('script')
  <script type="text/javascript">
    ...
    $('#editRoute').on('show.bs.modal', function(ev) {
        ...
        var link = $(ev.relatedTarget);
        var id = link.data('content');
        modal.find('.modal-body').load('/dashboard/editPlaneRoute/' + id);
    });
    $('#delRoute').on('show.bs.modal', function(ev) {
            var modal = $(this);
            var link = $(ev.relatedTarget);
            var id = link.data('content');
            modal.find('.modal-body').load('/dashboard/deletePlaneRoute/' + id);
    });
  </script>
